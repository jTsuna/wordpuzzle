    ...
    var WordFindGame = function () {
        ...
        var drawPuzzle = function (el, puzzle) {
            ...
            for (var i = 0, height = puzzle.length; i < height; i++) {
                ...
                for (var j = 0, width = row.length; j < width; j++) {
                    ...
                    output += '<button class="puzzleSquare" style="height: 50px; width: 50px; text-transform: uppercase;" x="' + j + '" y="' + i + '">';
                    output += row[j] || '&nbsp;';
                    output += '</button>';
                }
        ...
        */
        var drawWords = function (el, words) {
            ...
            var output = '<ul class="flex flex-col justify-center items-center" style="list-style-type: none;">';
            for (var i = 0, len = words.length; i < len; i++) {
                var word = words[i];
                output += '<li class="word ' + word + '"style="font: 1em sans-serif;">' + word;
            }
            output += '</ul>';
        ...
        */
        var startTurn = function () {
            $(this).addClass('text-white font-bold bg-gradient-to-l from-blue-500 to-indigo-500 focus:border-none rounded-full');
            startSquare = this;
        ...
        */
        var select = function (target) {
            ...
            while (backTo < selectedSquares.length) {
                $(selectedSquares[selectedSquares.length - 1]).removeClass('text-white font-bold bg-gradient-to-l from-blue-500 to-indigo-500 focus:border-none rounded-full');
            ...
            if (newOrientation) {
                ...
                if (lastSquare !== startSquare) {
                    $(lastSquare).removeClass('text-white font-bold bg-gradient-to-l from-blue-500 to-indigo-500 focus:border-none rounded-full');
        ...
        var playTurn = function (square) {
            ...
            for (var i = 0, len = wordList.length; i < len; i++) {
                if (wordList[i].indexOf(curWord + $(square).text()) === 0) {
                    $(square).addClass('text-white font-bold bg-gradient-to-l from-blue-500 to-indigo-500 focus:border-none rounded-full');
        ...
        var endTurn = function () {
            ...
            for (var i = 0, len = wordList.length; i < len; i++) {
                ...
                if (wordList[i] === curWord) {
                    $('text-white font-bold bg-gradient-to-l from-blue-500 to-indigo-500 focus:border-none').addClass('text-black');
                    wordList.splice(i, 1);
                    $('.' + curWord).addClass('flex justify-center line-through text-black bg-yellow-400');
                }

                if (wordList.length === 0) {
                    $('.puzzleSquare').addClass('bg-white text-white');
                    alert('Congratulations!');
                }
            }

            // reset the turn
            $('text-white font-bold bg-gradient-to-l from-blue-500 to-indigo-500 focus:border-none rounded-full').removeClass('text-white font-bold bg-gradient-to-l from-blue-500 to-indigo-500 focus:border-none rounded-full');
            startSquare = null;
            selectedSquares = [];
            curWord = '';
            curOrientation = null;
            ...
            */
            solve: function (puzzle, words) {
                ...
                for (var i = 0, len = solution.length; i < len; i++) {
                    ...
                    if (!$('.' + word).hasClass('flex justify-center line-through text-black bg-yellow-400')) {
                        for (var j = 0, size = word.length; j < size; j++) {
                            var nextPos = next(x, y, j);
                            $('[x="' + nextPos.x + '"][y="' + nextPos.y + '"]').addClass('solved');
                        }

                        $('.' + word).addClass('flex justify-center line-through text-black bg-yellow-400');
                    }
                }
...
$(function () {
    var words = ['functions', 'variables', 'indent', 'comments', 'justify', 'center', 'syntax', 'queen'];
    ...
    $('#solve').click(function () {
        wordfindgame.solve(gamePuzzle, word);
    });
